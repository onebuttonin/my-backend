...
class TransactionalMailController extends Controller
{
    public function sendOrderMail(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::error('Brevo template check failed', ['id' => $templateId, 'body' => method_exists($e, 'getResponseBody') ? $e->getResponseBody() : $e->getMessage()]);
            return response()->json(['message' => 'Template not accessible', 'details' => $e->getMessage()], 422);
        }
        ...
        try {
            ...
            $messageId = (is_object($result) && method_exists($result, 'getMessageId')) ? $result->getMessageId() : null;
            ...
            Log::error('Brevo send error', ['code' => $e->getCode(), 'body' => method_exists($e, 'getResponseBody') ? $e->getResponseBody() : $e->getMessage()]);
            return response()->json(['message' => 'Failed to send', 'details' => $e->getMessage()], 500);
        } catch (\Exception $e) {
